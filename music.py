...
async def _fetch_and_send_music(message: Message):
    ...
    try:
        ...
        if not message.reply_to_message \
            ...
            inform = ("Bu video Musiqi kateqoriyasına aid deyil, "
                      "linki cavab olaraq təkrar göndərə bilərsiniz "
                      "yükləməyə məcbur etme")
        ...
        if info_dict['duration'] > MUSIC_MAX_LENGTH:
            ...
            inform = ("Səs uzunluğu olduğuna görə bu endirilməyəcək "
                      "həddindən artıqdır `{}` bot tərəfindən təyin olunub"
                      .format(readable_max_length))
            await _reply_and_delete_later(message, inform,
                                          DELAY_DELETE_INFORM)
            return
        d_status = await message.reply_text("Yüklənir...", quote=True,
